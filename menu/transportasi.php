<?php
    ...
    function getno(){
        $sql = "Select IfNull(Max(idtransportasi)+1, 1) from transportasi";
...
?>
                        ...
                        <div class="col-md-5">
                            <div class="box box-primary">
                                <div class="box-header">
                                    <h3 class="box-title">Entri Data Transportasi</h3>
                                </div>
                                <div class="box-body">
                                    <form id="transportasiform" name="transportasiform" role="form" class="form-horizontal"
                                        method="post" action="data/simpantransportasi.php?act=new" novalidate="novalidate">
                                        <div class="form-group">
                                            <label for="idtransportasi" class="col-md-4 control-label">Id Transportasi</label>
                                            <div class="col-md-8">
                                                <input type="text" class="form-control" id="idtransportasi" name="idtransportasi"
                                                    placeholder="Id Transportasi" readonly value="<?php echo $no?>">
                                            </div>
                                        </div>
                                        <div class="form-group">
                                            <label for="namatransportasi" class="col-md-4 control-label">Nama Transportasi</label>
                                            <div class="col-md-8">
                                                <input type="text" class="form-control" id="namatransportasi" name="namatransportasi"
                        ...
                        <div class="col-md-7">
                            <div class="box box-primary">
                                ...
                                <div class="box-body table-responsive">
                                    <table id="datatransportasi" class="table table-bordered table-hover">
                                        <thead>
                                            <tr>
                                                <th class="center" style="width: 10%">No</th>
                                                <th class="center" style="width: 15%">Id Transportasi</th>
                                                <th style="width: 65%">Nama Transportasi</th>
        ...
        <script>
            ...
            function getdelete(idtransportasi){
                $.alerts.dialogClass = 'alert-blue';
                $.alerts.okButton = 'Yes';
                $.alerts.cancelButton = 'No';
                jConfirm('Anda yakin menghapusnya ?', 'Konfirmasi', function(r){
                    if(r === true){
                        window.location = 'data/simpantransportasi.php?act=delete&id='+idtransportasi;
                    }
                    else{return false;}
                });
            }
            jQuery(document).ready(function(){
                function No(){
                    var idtransportasi = document.getElementById('idtransportasi');
                    $.ajax({
                        dataType: 'json',
                        url: "data/maxtransportasi.php",
                        type: "GET",
                        success: function(data){
                            $.each(data, function(i, n){
                                var transportasi1 = n["idtransportasi"];
                                idtransportasi.value = transportasi1;
                            });
                        },
                        failure: function(){
                ...
                var test;
                test = $("#transportasiform").validate({
                    rules:{
                        idtransportasi:{required : true},
                        namatransportasi:{required : true}
                    },
                    messages:{
                        idtransportasi:{required: "Id Transportasi tidak boleh kosong"},
                ...
                datatransportasi = $("#datatransportasi").dataTable({
                    ...
                    "bProcessing": false,
                    "bServerSide": true,
                    "sAjaxSource": "data/datatransportasi.php",
                    "oLanguage": {
                    ...
                    "aoColumns": [
                        ...
                        {"bSortable": false, "sClass": "center", "mData": "idtransportasi"},
                        {"bSortable": false, "mData": "namatransportasi"},
                        {"bSortable": false, "sClass": "center", "fnRender": function(oObj){
                                return "<td>"+
                                       "<a href='edittransportasi-"+oObj.aData["Action"]+"' "+
                    ...
                    "fnDrawCallback": function( oSettings ){
                        jQuery('div#datatransportasi_length select, div#datatransportasi_filter input').addClass("form-control input-group");
                    },
                    "oTableTools": {
                        "aButtons": [
                            {
                ...
                jQuery('#transportasiform').on('submit', function(e){
                    e.preventDefault();
                    if(jQuery('#idtransportasi').val() === '' || jQuery('#namatransportasi').val() === '' ){
                        return false;
                    }
                    else{
                        jQuery(this).ajaxSubmit({
                            success: function(data){
                                if(data === 'true'){
                                    jQuery.gritter.add({
                                        title: 'Transportasi',
                                        text: 'Simpan Transportasi Sukses',
                                        sticky: false,
                                        time: ''
                                    });
                                    $.alerts.dialogClass = 'alert-blue';
                                    jAlert('Simpan Transportasi Sukses', 'Konfirmasi', function(){
                                        $.alerts.dialogClass = null;
                                        jQuery('#transportasiform').resetForm();
                                        reloadTabel(datatransportasi);
                                        No();
                                    });
                                }
                                else if(data === 'false'){
                                    jQuery.gritter.add({
                                        title: 'Transportasi',
                                        text: 'Simpan Transportasi Gagal',
                                        sticky: false,
                                        time: ''
                                    });
                                    $.alerts.dialogClass = 'alert-blue';
                                    jAlert('Simpan Transportasi Gagal', 'Konfirmasi', function(){
                                        $.alerts.dialogClass = null;
                                        No();
                                    });
                                }
