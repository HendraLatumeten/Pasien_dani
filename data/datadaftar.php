<?php
    ...
    * Get Where DataTables
    ...
    $sQuery1 = "
        ...
        FROM (";
    $sQuery1 .= "Select @row := @row+1 `no`, tahunajaran, kodedaftar, tanggaldaftar, "
        . "namadaftar, namastatus From pendaftaran d, status s, "
        . "(Select @row := 0) p Where s.idstatus = d.idstatus";
    $sQuery1 .= ") a";
    $sQuery1 .= " $sWhere
        $sOrder
        $sLimit";
    $result = mysql_query($sQuery1);
    $sQuery = "Select Count(p.kodedaftar) Jml From (Select @row := @row+1 `no`, tahunajaran, kodedaftar, tanggaldaftar, "
        . "namadaftar, namastatus From pendaftaran d, status s, "
        . "(Select @row := 0) p Where s.idstatus = d.idstatus) p ";
    $sQuery .= " $sWhere";
