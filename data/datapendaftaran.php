<?php
    ...
    session_start();
    $sql = "SELECT Null `no`, Null tahunajaran, Null kodedaftar, Null tanggaldaftar, Null namadaftar, "
        . "Null namastatus, Null Action From Dual Limit 0, 0";
    ...
    * Get Where DataTables
    ...
    $sQuery1 = "
        ...
        FROM (";
    $sQuery1 .= "Select @row := @row+1 `No`, tahunajaran, kodedaftar, tanggaldaftar, namadaftar, namastatus, "
        . "iddaftar Action From pendaftaran a, status s, "
        . "(Select @row := 0) p Where s.idstatus = a.idstatus";
    $sQuery1 .= ") a";
    $sQuery1 .= " $sWhere
        $sOrder
        $sLimit";
    $result = mysql_query($sQuery1);
    $sQuery = "Select Count(p.kodedaftar) Jml From (Select @row := @row+1 `No`, tahunajaran, kodedaftar, tanggaldaftar, namadaftar, namastatus, "
        . "iddaftar Action From pendaftaran a, status s, "
        . "(Select @row := 0) p Where s.idstatus = a.idstatus) p ";
    $sQuery .= " $sWhere";
