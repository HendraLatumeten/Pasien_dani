<?php
    ...
    session_start();
    $sql = "SELECT Null `no`, Null idpaud, Null namapaud From Dual Limit 0, 0";
    ...
    * Get Where DataTables
    ...
    $sQuery1 = "
        ...
        FROM (";
    $sQuery1 .= "Select @row := @row+1 `no`, idpaud, namapaud From paud k, "
        . "(Select @row := 0) p";
    $sQuery1 .= ") a";
    $sQuery1 .= " $sWhere
        $sOrder
    ...
    $sQuery = "Select Count(p.idpaud) Jml From (Select @row := @row+1 `no`, idpaud, namapaud From paud k, "
        . "(Select @row := 0) p) p ";
    $sQuery .= " $sWhere";
