<?php
    ...
    session_start();
    $sql = "SELECT Null `no`, Null idtk, Null namatk From Dual Limit 0, 0";
    ...
    * Get Where DataTables
    ...
    $sQuery1 = "
        ...
        FROM (";
    $sQuery1 .= "Select @row := @row+1 `no`, idtk, namatk From tk k, "
        . "(Select @row := 0) p";
    $sQuery1 .= ") a";
    $sQuery1 .= " $sWhere
        $sOrder
    ...
    $sQuery = "Select Count(p.idtk) Jml From (Select @row := @row+1 `no`, idtk, namatk From tk k, "
        . "(Select @row := 0) p) p ";
    $sQuery .= " $sWhere";
