<?php
    ...
    session_start();
    $sql = "SELECT Null `no`, Null idstatus, Null namastatus From Dual Limit 0, 0";
    ...
    * Get Where DataTables
    ...
    $sQuery1 = "
        ...
        FROM (";
    $sQuery1 .= "Select @row := @row+1 `no`, idstatus, namastatus From status k, "
        . "(Select @row := 0) p";
    $sQuery1 .= ") a";
    $sQuery1 .= " $sWhere
        $sOrder
    ...
    $sQuery = "Select Count(p.idstatus) Jml From (Select @row := @row+1 `no`, idstatus, namastatus From status k, "
        . "(Select @row := 0) p) p ";
    $sQuery .= " $sWhere";
