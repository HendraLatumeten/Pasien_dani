<?php
    ...
    session_start();
    $sql = "SELECT Null `no`, Null idkps, Null namakps From Dual Limit 0, 0";
    ...
    * Get Where DataTables
    ...
    $sQuery1 = "
        ...
        FROM (";
    $sQuery1 .= "Select @row := @row+1 `no`, idkps, namakps From kps k, "
        . "(Select @row := 0) p";
    $sQuery1 .= ") a";
    $sQuery1 .= " $sWhere
        $sOrder
    ...
    $sQuery = "Select Count(p.idkps) Jml From (Select @row := @row+1 `no`, idkps, namakps From kps k, "
        . "(Select @row := 0) p) p ";
    $sQuery .= " $sWhere";
