<?php
    ...
    $sql = "Select Null `No`, a.namauser, concat(namadepan, ' ', namabelakang) nama, email From userlogin a Limit 0, 0";
    ...
    * Get Where DataTables
    ...
    $sQuery1 = "
        ...
        FROM (";
    $sQuery1 .= "Select @row := @row + 1 `No`, u.namauser, Concat(IfNull(namadepan, ''), ' ', IfNull(namabelakang, '')) nama, email "
        . "From userlogin u, (Select @row := 0) p Where idgroup = '$idgroup'";
    $sQuery1 .= ") a";
    $sQuery1 .= " $sWhere
        $sOrder
        $sLimit";
    $result = mysql_query($sQuery1);
    $sQuery = "Select Count(p.email) Jml From (Select @row := @row + 1 `No`, u.namauser, Concat(namadepan, ' ', namabelakang) nama, email "
        . "From userlogin u, (Select @row := 0) p Where idgroup = '$idgroup') p ";
    $sQuery .= " $sWhere";
