<?php
    ...
    session_start();
    $sql = "Select Null `No`, Null iddaftar, Null kodedaftar, Null namadaftar, Null statusdaftar "
        . "From Dual Limit 0, 0";
    ...
    * Get Where DataTables
    ...
    $sQuery1 = "
        ...
        From (";
    $sQuery1 .= "Select @row := @row+1 `No`, a.* From (Select Distinct p.iddaftar, kodedaftar, namadaftar, "
        . "Case When h.iddaftar Is Null Then 'Belum Test' Else 'Sudah Test' End statusdaftar "
        . "From pendaftaran p Left Join hasiltest h On h.iddaftar = p.iddaftar) a, (Select @row := 0) p";
    $sQuery1 .= ") a";
    $sQuery1 .= " $sWhere
        $sOrder
        $sLimit";
    $result = mysql_query($sQuery1);
    $sQuery = "Select Count(p.iddaftar) Jml From (Select @row := @row+1 `No`, a.* From (Select Distinct p.iddaftar, kodedaftar, namadaftar, "
        . "Case When h.iddaftar Is Null Then 'Belum Test' Else 'Sudah Test' End statusdaftar "
        . "From pendaftaran p Left Join hasiltest h On h.iddaftar = p.iddaftar) a, (Select @row := 0) p) p ";
    $sQuery .= " $sWhere";
